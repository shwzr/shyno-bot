...
module.exports = {
  ...
  async run(bot, message, args) {
    try {
      ...
      try {
        await user.send(`Tu as été unban du serveur par ${message.user.tag} pour la raison : \`${reason}\``);
      } 
      catch (err) {} await message.reply(`${message.user} a unban ${user.tag} pour la raison : \`${reason}\``);
